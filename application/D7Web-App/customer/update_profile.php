...
if (isset($_POST['submit'])) {    
    ...
    if (!empty($complete_name)) {
        $update_name = $conn->prepare("UPDATE `customers` SET complete_name = ? WHERE customer_id = ?");
        $update_name->execute([$complete_name, $customer_id]);
    ...
    if (!empty($phone_number)) {
        if (!preg_match('/^[0-9]{11}+$/', $phone_number)) {
            ...
            $update_number = $conn->prepare("UPDATE `customers` SET phone_number = ? WHERE customer_id = ?");
            $update_number->execute([$phone_number, $customer_id]);
    ...
    if (!empty($email_address)) {
        $select_email = $conn->prepare("SELECT * FROM `customers` WHERE email_address = ?");
        $select_email->execute([$email_address]);
        if ($select_email->rowCount() > 0) {
            $msg = "<div class='alert alert-danger'>Email already exist.</div>";
        } else {
            $update_email = $conn->prepare("UPDATE `customers` SET email_address = ?, verification = ?  WHERE customer_id = ?");
            $update_email->execute([$email_address, $verification, $customer_id]);
            ...
            try {
                ...
                $mail->Body = 'Here is the verification link <b><a href="http://localhost/D7Web-App/customer/update_profile.php?verification=' . $verification . '">
                    http://localhost/D7Web-App/customer/update_profile.php?verification=' . $verification . '</a></b>';
                $mail->send();
    ...
    if (!empty($profile_picture)) {
        if($profile_picture_size > 2000000){
            ...
            $update_profile = $conn->prepare("UPDATE `customers` SET profile_picture = ? WHERE customer_id = ?");
            $update_profile->execute([$profile_picture, $customer_id]);
    ...
    if (!empty($new_pass) || !empty($confirm_pass)) {
        $select_prev_pass = $conn->prepare("SELECT password FROM `customers` WHERE customer_id = ?");
        $select_prev_pass->execute([$customer_id]);
            ...
            if ($new_pass != $confirm_pass) {
                ...
                if (!preg_match('/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,32}$/', $new_pass)) {
                    ...
                    if (sha1($new_pass) != $old_pass) {
                        ...
                        $update_pass->execute([sha1($new_pass), $customer_id]);
...
<section class="form-container update-form">
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <h2>Change Name:</h2>
      <input type="text" name="complete_name" class="box" maxlength="32" placeholder="<?= $fetch_profile['complete_name']; ?>" >
      <h2>Change Email Address:</h2>
      <input type="email" name="email_address" class="box" placeholder="<?= $fetch_profile['email_address']; ?>"  maxlength="32" oninput="this.value = this.value.replace(/\s/g, '')">
      <h2>Change Phone Number:</h2>
      <input type="tel" name="phone_number" class="box" placeholder="<?= $fetch_profile['phone_number']; ?>"  maxlength="11">
      <h2>Old Password:</h2>
