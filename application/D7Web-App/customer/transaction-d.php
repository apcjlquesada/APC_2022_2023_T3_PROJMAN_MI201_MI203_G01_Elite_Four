...
<?php
    ...
    $sql = "SELECT * FROM `reservations` WHERE CONCAT(`customer_name`, `service_type`, `car_model`, `customer_number`) LIKE :search_query AND `status` = 'CANCELLED' AND `customer_id` = :customer_id ";
    ...
    $total_records = $conn->prepare("SELECT COUNT(*) FROM `reservations` WHERE  CONCAT(`customer_name`, `service_type`, `car_model`, `customer_number`) LIKE :search_query AND `status` = 'CANCELLED' AND `customer_id` = :customer_id");
    ...
    ?>
    <h1 class="title"> Cancelled Reservations Details</h1>
    <div class="search-container">
    <form action="transaction-d.php" method="GET" class="search" id="search-form" style="bottom: 14rem;" >
